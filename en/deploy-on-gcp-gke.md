...
1. Create a GKE cluster and a default node pool:
    ...
    ```shell
    gcloud container clusters create tidb --region us-west1 --machine-type n1-standard-2 --num-nodes=1
    ```
...
2. Create separate node pools for PD, TiKV, and TiDB:
    ...
    ```shell
    gcloud container node-pools create pd --cluster tidb --machine-type n1-standard-2 --num-nodes=1 \
        --node-labels=dedicated=pd --node-taints=dedicated=pd:NoSchedule
    gcloud container node-pools create tikv --cluster tidb --machine-type n1-highmem-2 --num-nodes=1 \
        --node-labels=dedicated=tikv --node-taints=dedicated=tikv:NoSchedule
    gcloud container node-pools create tidb --cluster tidb --machine-type n1-standard-2 --num-nodes=1 \
